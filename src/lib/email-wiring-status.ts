...
export const WIRING_STATUS: Record<string, WiringEntry> = {
  ...
  'AUTH-07': { status: 'deferred', reason: 'MFA enroll/disable in adminSessions does not notify yet' },
  'AUTH-08': { status: 'deferred', reason: 'login records login_history but does not email new-device' },
  'AUTH-09': { status: 'deferred', reason: 'MFA lockout sets mfa_locked_until but does not email' },
  'AUTH-10': { status: 'wired', where: 'routes/admin-lifecycle.ts invitations → inviteEmail' },
  'AUTH-11': { status: 'deferred', reason: 'approval flow exists; access-granted email not wired' },
