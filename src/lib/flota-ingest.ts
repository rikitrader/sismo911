...
import { validateGps, isImpossibleJump, type GpsInput } from './flota-gps';
...
export async function ingestGps(env: Env, ctx: IngestCtx, payload: GpsInput, now: number): Promise<IngestResult> {
  const actorId = `unit:${ctx.unitId}`;

  const v = validateGps(payload, now);
  if (!v.ok) {
    await audit(env, { actorId, unitId: ctx.unitId, action: 'gps.reject', meta: { reason: v.error } });
  ...
  if (!unit) {
    await audit(env, { actorId, unitId: ctx.unitId, action: 'gps.reject', meta: { reason: 'unit_not_found' } });
    return { ok: false, error: 'unit_not_found' };
  }
  if (unit.status !== 'active') {
    await audit(env, { actorId, unitId: ctx.unitId, action: 'gps.reject', meta: { reason: 'unit_inactive' } });
  ...
  if (isImpossibleJump(prev, v.value)) {
    await audit(env, { actorId, unitId: ctx.unitId, action: 'gps.reject', meta: { reason: 'impossible_jump' } });
  ...
  await env.DB.prepare(
    ...
    v.value.speed_mps, v.value.battery_pct, 'phone', v.value.recorded_at, now,
  ).run();

  await audit(env, { actorId, unitId: ctx.unitId, action: 'gps.ingest', meta: { loc: id, accuracy: v.value.accuracy_m } });
