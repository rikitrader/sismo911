...
import { rateLimit, validLatLon, nameHasSpam, textHasLink } from '../lib/security';
...
mascotas.get('/api/mascotas/:id', async (c) => {
  ...
  const r: any = await c.env.DB.prepare(
    `SELECT id, kind, category, title, description, city, state, area, lat, lng,
            ${op ? 'contact,' : ''} status, photo_url, tags, created_at,
...
// Lands moderation='pending'; visible after an operator approves. (Photo is an
// optional URL for now; binary upload to R2 is a tracked follow-up.)
mascotas.post('/api/mascotas/report', async (c) => {
  ...
  const lng = validLatLon(b.lat, b.lng) ? Number(b.lng) : null;
  await c.env.DB.prepare(
    `INSERT INTO rav_reports (id, kind, category, title, description, city, state, area, lat, lng, contact, status, photo_url,
    ...
    lat, lng, clamp(b.contact, 120) || null, clamp(b.photo_url, 600) || null, nowIso, nowIso, nowIso, category).run();
  await audit(c, 'mascota.report', { id, category });
