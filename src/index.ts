...
import { contactsPersonal } from './routes/contacts-personal';
...
import { sendas } from './routes/sendas';
...
import { dedupeReview } from './routes/dedupe-review';
...
app.route('/api/send-as', sendas);    // "Send as ricardo@sismo911.com" outbound mailer via the send_email binding; every endpoint gated ops:console; + Workers-AI drafter at /draft
...
app.route('/api/impact', impact);    // public, auditable impact metrics (personas ayudadas) — counts only, no PII
...
app.route('/api/admin/dedupe', dedupeReview); // dedupe candidate/conflict review queue (ops:console)
...
app.get('/edificio/:id', (c) => c.env.ASSETS.fetch(new Request(new URL('/edificio.html', c.req.url))));
