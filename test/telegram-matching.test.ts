...
describe('resolveQuery end-to-end (no_match / multiple / single)', () => {
  ...
  it('OPERATORS get the FULL list of matches (public gets asked to narrow)', async () => {
    const cmd = { kind: 'buscar' as const, lang: 'es' as const, name: 'Carlos Ramirez', raw: '' };
    const asPublic = await resolveQuery(env(data), cmd, { canSeeSensitive: false, role: 'public', nowMs: NOW });
    const asAdmin = await resolveQuery(env(data), cmd, { canSeeSensitive: true, role: 'admin', nowMs: NOW });
    expect(asPublic.kind).toBe('multiple');
    expect(asAdmin.kind).toBe('list');
    expect(asAdmin.kind === 'list' && asAdmin.records.length).toBe(2);
  });
