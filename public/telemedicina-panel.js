/* SISMO911 Telemedicina — doctor panel (v2).
   ...
   blocks → that's what generates patient slots), Calendario. */
(function () {
  'use strict';
  ...
  const TYPES = [
    { key: 'video', label: 'Videoconsulta' }, { key: 'followup', label: 'Seguimiento' }, { key: 'urgent', label: 'Urgente' },
    { key: 'mental_health', label: 'Salud mental' }, { key: 'refill', label: 'Receta' },
  ];
  ...
  const ST_BADGE = { scheduled: ['#dcfce7', '#166534'], checked_in: ['#dbeafe', '#1e40af'], waiting_room: ['#fef3c7', '#92600a'], in_progress: ['#cffafe', '#0e7490'], completed: ['#e5e7eb', '#374151'], cancelled: ['#fee2e2', '#991b1b'], no_show: ['#fee2e2', '#991b1b'] };
  const TERMINAL = ['completed', 'cancelled', 'no_show'];
  // Allowed doctor actions per current status.
  const ACTIONS = {
  ...
  async function loadAppointments() {
    ...
    $('pane-appts').innerHTML = (active.length ? active.map(apptRow).join('') : '<div class="empty">No tienes citas activas.</div>') +
      (past.length ? `<h2 class="sec" style="margin-top:20px;font-size:15px">Historial</h2>${past.map(apptRow).join('')}` : '');
    wireApptButtons();
  }
  ...
  function apptRow(a) {
    ...
    let btns = '';
    if (!term && a.video_url) btns += `<a class="videolink" href="${esc(a.video_url)}" target="_blank" rel="noopener">🎥 Entrar</a>`;
    btns += acts.map(([to, label, cls]) => `<button class="act ${cls}" data-id="${esc(a.id)}" data-to="${to}">${label}</button>`).join('');
    if (!term) btns += `<a class="act ghost" style="text-decoration:none" href="/api/telemedicina/appt/${esc(a.id)}/ics?t=${encodeURIComponent(TOKEN)}">📅 .ics</a>`;
    return `<div class="row ${term ? 'term' : ''}">
      <div class="top">
        <div><div class="name">${esc(a.patient_name)} <span class="badge" style="background:${bd[0]};color:${bd[1]}">${ST_LABEL[a.status]}</span></div>
        <div class="meta">${esc(typeL(a.appt_type))} · ${esc(specL(a.specialty))}${contact ? ' · ' + esc(contact) : ''}</div></div>
        <div class="meta" style="text-align:right">📅 ${tFmt(a.start_ms)}</div>
      </div>
      ${a.reason ? `<div class="desc">${esc(a.reason)}</div>` : ''}
      ${a.insurance_provider ? `<div class="meta" style="margin-top:6px">Seguro: ${esc(a.insurance_provider)}</div>` : ''}
      <div class="actions">${btns}</div>
    </div>`;
  }

  function wireApptButtons() {
    $('pane-appts').querySelectorAll('button.act[data-to]').forEach((b) => b.addEventListener('click', async () => {
  ...
  async function loadAvailability() {
    ...
    // First window per weekday → UI row. Cold start: suggest Mon–Fri 09:00–17:00.
    const byDay = {};
    ...
    $('av_days').innerHTML = DAY_ORDER.map((d) => {
      ...
      return `<div class="dayrow ${on ? '' : 'off'}" data-day="${d}">
        <label style="display:flex;align-items:center;gap:7px;width:108px"><input type="checkbox" class="avday" ${on ? 'checked' : ''}> <span class="dn" style="width:auto">${DAY_NAMES[d]}</span></label>
        <span class="times"><input class="f avfrom" type="time" value="${from}" style="width:118px"> <span style="color:var(--muted)">–</span> <input class="f avto" type="time" value="${to}" style="width:118px"></span>
      </div>`;
    }).join('');
  ...
  function renderBlocks(blocks) {
    if (!blocks.length) { $('blk_list').innerHTML = '<div class="meta" style="padding:8px 0;color:var(--muted)">Sin fechas bloqueadas.</div>'; return; }
    $('blk_list').innerHTML = blocks.map((b) => {
      ...
      return `<div class="blkrow"><b>${esc(b.date)}</b> · ${range}${b.reason ? ' · ' + esc(b.reason) : ''}<button class="act danger" data-blk="${esc(b.id)}" style="margin-left:auto;padding:6px 11px">Quitar</button></div>`;
    }).join('');
    ...
    for (const a of appts) {
      ...
      html += `<div class="cal-item">
        <div style="display:flex;gap:12px;align-items:center">
          ...
          <div><div class="name">${esc(a.patient_name)} <span class="badge" style="background:${bd[0]};color:${bd[1]}">${ST_LABEL[a.status]}</span></div><div class="meta">${esc(typeL(a.appt_type))} · ${esc(specL(a.specialty))}</div></div>
        </div>
        ${a.video_url ? `<a class="videolink" href="${esc(a.video_url)}" target="_blank" rel="noopener">🎥 Entrar</a>` : ''}
      </div>`;
    }
